...
const AirdropsTestnetPage = () => {
  const [tasks, setTasks] = useState([]);
  const router = useRouter();

  useEffect(() => {
    async function fetchTasks() {
      const response = await fetch('/api/testnet-tasks'); // Endpoint untuk mendapatkan data dari MongoDB
  ...
  return (
    <Box sx={{ padding: 4 }}>
      ...
      {tasks.length === 0 ? (
        <Typography sx={{ color: 'white' }}>Tidak ada tugas airdrop testnet.</Typography>
      ) : (
        tasks.map((task) => (
...
export default AirdropsTestnetPage;
